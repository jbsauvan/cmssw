...
## Publication on web
publish_plots = True
publication_dir = "/afs/cern.ch/user/j/jsauvan/www/H2Taus/FakeRate/BackgroundEstimation_WithFakeFactorUncertainty/"
...
histo_file_template_name = histo_base_dir+'/{SAMPLE}/'+histo_version+'/fakerates_MuTau_{SAMPLE}_FFStatUnc.root'
histo_template_name = '{DIR}hFakeRate_{SEL}_{VAR}_vs_match5{UNCERT}' ## '_vs_match5' is for gen_match=6
...
## Output 
plot_dir = "fakeplots_withFakeFactorUncertainty/"
if not os.path.exists(plot_dir):
    os.makedirs(plot_dir)

outFile = ROOT.TFile.Open(plot_dir+"/histos.root", "RECREATE")
        ...
        for fake_factor in fake_factors:
            ...
            for variable in variables:
                ...
                for config in configs_signal:
                    config.histo_name = histo_template_name.format(DIR='',SEL=global_selection+signal_selections[fake_factor],VAR=variable.name,UNCERT='')
                    config.name = "Data"
                ...
                for config in configs_inverted:
                    config.histo_name = histo_template_name.format(DIR=fake_factor+'/',SEL=global_selection+inverted_selections[fake_factor],VAR=variable.name,UNCERT='_FFStatUnc')
                # append both configs
