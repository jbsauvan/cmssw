...
inc_sig_tau = Cut('!veto_dilepton')
inc_sig_tau &= Cut('!veto_thirdlepton')
inc_sig_tau &= Cut('!veto_otherlepton')
...
inc_sig_mu = Cut('l1_reliso05<0.1')
inc_sig_mu &= Cut('l1_muonid_medium>0.5')
inc_sig_mu &= Cut('l1_pt>19')

inc_sig = inc_sig_mu & inc_sig_tau

cuts['HighMT_OS'] = inc_sig & Cut('l1_charge != l2_charge ') & Cut('mt>40')
cuts['HighMT_SS'] = inc_sig & Cut('l1_charge == l2_charge ') & Cut('mt>40')
...
for name,cut in cuts.items():
    cfg_example = HistogramCfg(name='example', var=None, cfgs=samples, cut=str(cut), lumi=int_lumi)
